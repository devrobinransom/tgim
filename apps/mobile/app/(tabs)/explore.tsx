    ...
    <Screen>
      ...
      {view === 'map' ? (
        <>
          ...
          <Text style={{ color: theme.textMuted, fontSize: 11, textAlign: 'center' }}>{t('tapMarkerHint')}</Text>
        </>
      ) : (
        <Text style={{ color: theme.textMuted, fontSize: 12 }}>{`${visible.length} ${t('issuesInArea')}`}</Text>
      )}
...
function ViewToggle({ active, icon, label, onPress }: { active: boolean; icon: 'map' | 'list'; label: string; onPress: () => void }) {
  return (
    ...
    >
      ...
      <Text style={{ color: active ? theme.accent : theme.textMuted, fontSize: 12, fontWeight: '700' }}>{label}</Text>
    </Pressable>
  );
}