...
import { 
  ...
  DeliveryUpdate, AuditEvent, UserRole, IssueSeverity, PrivacyLevel, PromiseStatus 
} from '@tgim/shared';
...
export const dbService = {
  ...
  issues: {
    ...
    findMany: async (filters: { area_id?: string; category?: string }): Promise<Issue[]> => {
      if (isPrismaEnabled && prisma) {
        ...
        `;
        if (filters.area_id) sql += ` AND area_id = '${filters.area_id}'::uuid`;
        if (filters.category) sql += ` AND category = '${filters.category}'`;
        return prisma.$queryRawUnsafe(sql) as any;
      }
  ...
  manifestos: {
    ...
    publish: async (id: string): Promise<Manifesto> => {
      if (isPrismaEnabled && prisma) {
  ...
  partyPromises: {
    ...
    findMany: async (filters: { party_id?: string; status?: PromiseStatus }): Promise<PartyPromise[]> => {
      if (isPrismaEnabled && prisma) {
        return prisma.partyPromise.findMany({ where: filters }) as any;
      }
