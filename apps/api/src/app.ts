...
export function buildApp() {
  ...
  app.post('/api/v1/manifesto/generate', async (request, reply) => {
    ...
    try {
      ...
      const timeHorizons: ('100-day' | '1-year' | '3-year' | '5-year')[] = ['100-day', '1-year', '3-year', '5-year'];
  ...
  app.get('/api/v1/manifesto/:areaId', async (request, reply) => {
    ...
    if (!manifesto) return reply.status(404).send({ error: 'No manifesto found for this area' });
