...
import {
  CreateIssueSchema, SubmitVerificationSchema,
  AdoptPromiseSchema, AddDeliveryUpdateSchema
  , ApplyVolunteerSchema, ReviewVolunteerSchema, PublishManifestoSchema,
  CreateDisputeSchema, ResolveDisputeSchema, NotificationPreferenceSchema,
  ModerationActionSchema
  , UploadEvidenceSchema
  , AssignPartyMemberSchema
  , AssignVerificationSchema, UpdateVerificationAssignmentSchema,
  UpdatePartyPromiseSchema
  , PaginationSchema
  , CreateAuthoritySchema, LinkExternalCaseSchema, UpdateExternalCaseSchema, Open311CreateRequestSchema,
  SubmitToAuthoritySchema, CreateExternalCaseAppealSchema, AddExternalCaseDocumentSchema,
  CreatePromiseMilestoneSchema, UpdatePromiseMilestoneSchema, CitizenPromiseVerdictSchema,
  PincodeGeocodeSchema
  , CreateOrganizationSchema, CreateOrganizationInvitationSchema, AcceptOrganizationInvitationSchema, GrantIssueSharingConsentSchema, CreateScopeGrantSchema
} from '@tgim/shared';
...
export function buildApp() {
  ...
  app.addHook('onRequest', async (request, reply) => {
    ...
    let bucket: { count: number; resetAt: number; remaining?: number };
    if (distributed) {
      try {
    ...
    if (bucket.count > limit) return reply.status(429).send({ error: { code: 'rate_limited', message: 'Too many requests' } });
  });
  ...
  const publishJob = async (eventType: import('./jobs-contract.js').JobType, entityType: string, entityId: string, payload: Record<string, unknown>) =>
    jobPublisher.publish({ event_id: crypto.randomUUID(), event_type: eventType, entity_type: entityType, entity_id: entityId, payload, schema_version: 1, occurred_at: new Date() });
  const invitationHash = (token: string) => createHash('sha256').update(token).digest('hex');
  const requireOrganizationAdmin = async (organizationId: string, actorId: string, isPlatformAdmin: boolean) => {
    if (isPlatformAdmin) return true;
    const membership = await dbService.organizations.findMembership(organizationId, actorId);
    return membership?.role === 'owner' || membership?.role === 'admin';
  };
  const requireScope = async (actor: { id: string; role: string }, reply: any, scopeType: import('@tgim/shared').ActorScopeType, scopeId: string, capability: string) => {
    if (actor.role === 'platform_admin') return { id: 'platform-admin' };
  ...
  const promiseAccountability = async (promiseId: string) => {
    const promise = await dbService.partyPromises.findUnique(promiseId); if (!promise) return null;
    ...
    return { promise, milestones, delivery_updates, source_cluster_ids: sourceClusterIds, official_cases: official_cases.map(toPublicExternalCase), public_disputes: disputes.map(toPublicDisputeOutcome).filter((item): item is NonNullable<typeof item> => item !== null), outcome: { score, label, milestone_progress, evidence_strength, timeliness, citizen_confidence, verdict_counts, calculated_at: new Date() } };
  };
  ...
  app.get('/ready', async (_request, reply) => {
    try {
      const prisma = dbService.getPrisma();
      if (process.env.NODE_ENV === 'production' && !prisma) return reply.status(503).send({ status: 'not_ready', reason: 'database_not_configured' });
      if (process.env.NODE_ENV === 'production' && !isSovereignMode()) return reply.status(503).send({ status: 'not_ready', reason: 'sovereignty_mode_required' });
      ...
      if (process.env.NODE_ENV === 'production' && isSovereignMode()) {
        ...
        if (missing.length) return reply.status(503).send({ status: 'not_ready', reason: 'sovereign_dependencies_missing', missing });
  ...
  app.get('/api/v1/media/:id.:extension', async (request, reply) => {
    const { id } = request.params as { id: string; extension: string };
    const object = readLocalEvidence(id);
    if (!object) return reply.status(404).send({ error: 'Evidence not found' });
  ...
  app.post('/api/v1/ai/speech-to-text', async (request, reply) => {
    ...
    if (!actor) return;
    const body = request.body as { audio_base64?: unknown; media_type?: unknown; language?: unknown };
  ...
  app.post('/api/v1/organizations/:id/invitations', async (request, reply) => {
    ...
    if (!organization || !organization.active) return reply.status(404).send({ error: 'Active organization not found' });
    if (!(await requireOrganizationAdmin(id, actor.id, actor.role === 'platform_admin'))) return reply.status(403).send({ error: 'Only an owner or admin of this organization can invite people' });
  ...
  app.post('/api/v1/organization-invitations/accept', async (request, reply) => {
    ...
    if (!invitation || invitation.accepted_at || invitation.expires_at <= new Date()) return reply.status(404).send({ error: 'Invitation is invalid, expired, or already accepted' });
    const isDevelopmentDemo = process.env.NODE_ENV !== 'production' && process.env.DEMO_AUTH_ENABLED !== 'false';
    if (!isDevelopmentDemo && actor.email?.trim().toLowerCase() !== invitation.invitee_email) return reply.status(403).send({ error: 'Sign in with the email address that received this invitation' });
  ...
  app.post('/api/v1/scope-grants', async (request, reply) => {
    const actor = await requireActor(request, reply, ['platform_admin']); if (!actor) return;
    const parsed = CreateScopeGrantSchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send({ error: parsed.error.flatten() });
    ...
    if (endsAt && endsAt <= startsAt) return reply.status(400).send({ error: { code: 'invalid_grant_window', message: 'ends_at must be after starts_at' } });
  ...
  app.get('/api/v1/scope-grants/mine', async (request, reply) => {
    const actor = await requireActor(request, reply); if (!actor) return;
    return dbService.scopeGrants.findForActor(actor.id);
  });

  app.post('/api/v1/scope-grants/:id/revoke', async (request, reply) => {
    const actor = await requireActor(request, reply, ['platform_admin']); if (!actor) return;
    const grant = await dbService.scopeGrants.revoke((request.params as { id: string }).id);
  ...
  app.post('/api/v1/geocode/pincode', async (request, reply) => {
    ...
    if (!pincode) return reply.status(404).send({ error: 'No pincode boundary contains the given coordinates' });
    await dbService.audit.log({
  ...
  app.get('/api/v1/authorities', async (request) => {
    const { areaId, category } = request.query as { areaId?: string; category?: string };
  ...
  app.get('/api/v1/authorities/route', async (request, reply) => {
    const { areaId, category, latitude, longitude } = request.query as { areaId?: string; category?: string; latitude?: string; longitude?: string };
  ...
  app.post('/api/v1/authorities', async (request, reply) => {
    const actor = await requireActor(request, reply, ['platform_admin']); if (!actor) return;
    const parsed = CreateAuthoritySchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send({ error: parsed.error.flatten() });
  ...
  app.post('/open311/v2/requests.json', async (request, reply) => {
    const actor = await requireActor(request, reply, ['citizen', 'volunteer', 'platform_admin']); if (!actor) return;
    const parsed = Open311CreateRequestSchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send([{ code: 400, description: parsed.error.issues.map(issue => issue.message).join('; ') }]);
    ...
    if (!authority) return reply.status(404).send([{ code: 404, description: 'Unknown service_code' }]);
  ...
  app.get('/open311/v2/requests/:id.json', async (request, reply) => {
    const { id } = request.params as { id: string }; const issue = await dbService.issues.findUnique(id); if (!issue) return reply.status(404).send([{ code: 404, description: 'Service request not found' }]);
    const cases = await dbService.externalCases.findByIssue(id); const latest = cases[0];
  ...
  app.get('/api/v1/issues/page', async (request, reply) => {
    const query = request.query as { page?: string; page_size?: string; areaId?: string; category?: string; status?: string; search?: string };
    const pagination = PaginationSchema.safeParse(query); if (!pagination.success) return reply.status(400).send({ error: pagination.error.flatten() });
    ...
    if (query.search) { const term = query.search.toLowerCase(); issues = issues.filter(issue => issue.description.toLowerCase().includes(term)); }
    const { page, page_size } = pagination.data; return { items: issues.slice((page - 1) * page_size, page * page_size).map(publicIssue), page, page_size, total: issues.length, last_updated: new Date() };
  });

  app.get('/api/v1/issues/:id', async (request, reply) => {
    const { id } = request.params as { id: string };
    const issue = await dbService.issues.findUnique(id);
    if (!issue || !isPublicIssue(issue)) return reply.status(404).send({ error: 'Issue not found' });
  ...
  app.get('/api/v1/public/clusters/:id', async (request, reply) => {
    ...
    if (!cluster || !isPublicVisibility(cluster)) return reply.status(404).send({ error: 'Issue cluster not found' });
    ...
    const severity_mix = issues.reduce<Partial<Record<string, number>>>((counts, issue) => {
  ...
  app.get('/api/v1/issues/:id/accountability', async (request, reply) => {
    const { id } = request.params as { id: string }; const issue = await dbService.issues.findUnique(id); if (!issue || !isPublicIssue(issue)) return reply.status(404).send({ error: 'Issue not found' });
    const official_cases = await dbService.externalCases.findByIssue(id);
  ...
  app.post('/api/v1/issues/:id/external-cases', async (request, reply) => {
    const actor = await requireActor(request, reply, ['department_officer', 'platform_moderator', 'platform_admin']); if (!actor) return;
    const { id } = request.params as { id: string }; const issue = await dbService.issues.findUnique(id); if (!issue) return reply.status(404).send({ error: 'Issue not found' });
    const parsed = LinkExternalCaseSchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send({ error: parsed.error.flatten() });
    const authority = await dbService.authorities.findUnique(parsed.data.authority_id); if (!authority) return reply.status(404).send({ error: 'Authority not found' });
    const grant = await requireScope(actor, reply, 'authority', authority.id, 'external_case.link'); if (!grant) return;
  ...
  app.patch('/api/v1/external-cases/:id', async (request, reply) => {
    const actor = await requireActor(request, reply, ['department_officer', 'platform_admin']); if (!actor) return;
    const { id } = request.params as { id: string }; if (!(await dbService.externalCases.findUnique(id))) return reply.status(404).send({ error: 'External case not found' });
    const parsed = UpdateExternalCaseSchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send({ error: parsed.error.flatten() });
  ...
  app.post('/api/v1/issues/:id/sharing-consents', async (request, reply) => {
    ...
    if (issue.reporter_id !== actor.id) return reply.status(403).send({ error: 'Only the report author can grant recipient sharing consent' });
  ...
  app.post('/api/v1/issues/:id/submit-to-authority', async (request, reply) => {
    const actor = await requireActor(request, reply, ['department_officer', 'platform_admin']); if (!actor) return;
    const { id } = request.params as { id: string }; if (!(await dbService.issues.findUnique(id))) return reply.status(404).send({ error: 'Issue not found' });
    const parsed = SubmitToAuthoritySchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send({ error: parsed.error.flatten() });
    const authority = await dbService.authorities.findUnique(parsed.data.authority_id); if (!authority) return reply.status(404).send({ error: 'Authority not found' });
    const grant = await requireScope(actor, reply, 'authority', authority.id, 'external_case.submit'); if (!grant) return;
    if (!(await dbService.reportSharingConsents.findActive(id, parsed.data.authority_id))) return reply.status(409).send({ error: 'The report author has not granted active consent to share this report with this authority' });
  ...
  app.post('/api/v1/external-cases/:id/documents', async (request, reply) => {
    const actor = await requireActor(request, reply, ['department_officer', 'platform_admin']); if (!actor) return;
    const { id } = request.params as { id: string }; if (!(await dbService.externalCases.findUnique(id))) return reply.status(404).send({ error: 'External case not found' });
    const parsed = AddExternalCaseDocumentSchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send({ error: parsed.error.flatten() });
    const item = await dbService.externalCaseDocuments.create({ external_case_id: id, ...parsed.data });
    await dbService.audit.log({ actor_id: actor.id, event_type: 'external_case.document_add', target_table: 'external_grievance_cases', target_id: id, payload: { document_id: item.id, is_public: item.is_public } }); return reply.status(201).send(item);
  });

  app.get('/api/v1/external-cases/:id/documents', async (request) => dbService.externalCaseDocuments.findByCase((request.params as { id: string }).id, true));

  app.post('/api/v1/external-cases/:id/appeals', async (request, reply) => {
    const actor = await requireActor(request, reply, ['citizen', 'volunteer', 'platform_moderator', 'platform_admin']); if (!actor) return;
    const { id } = request.params as { id: string }; if (!(await dbService.externalCases.findUnique(id))) return reply.status(404).send({ error: 'External case not found' });
    const parsed = CreateExternalCaseAppealSchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send({ error: parsed.error.flatten() });
    ...
    await dbService.audit.log({ actor_id: actor.id, event_type: 'external_case.appeal', target_table: 'external_grievance_cases', target_id: id, payload: { appeal_id: appeal.id } }); return reply.status(201).send(appeal);
  });
  ...
  app.post('/api/v1/verification/assignments', async (request, reply) => {
    const actor = await requireActor(request, reply, ['platform_moderator', 'platform_admin']); if (!actor) return;
    const parsed = AssignVerificationSchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send({ error: parsed.error.flatten() });
    ...
    await dbService.audit.log({ actor_id: actor.id, event_type: 'verification.assign', target_table: 'issue_clusters', target_id: parsed.data.cluster_id, payload: { volunteer_id: parsed.data.volunteer_id, assignment_id: assignment.id } }); return reply.status(201).send(assignment);
  });

  app.get('/api/v1/verification/assignments', async (request, reply) => {
    const actor = await requireActor(request, reply, ['volunteer', 'platform_moderator', 'platform_admin']); if (!actor) return;
  ...
  app.post('/api/v1/verification/assignments/:id/status', async (request, reply) => {
    const actor = await requireActor(request, reply, ['volunteer']); if (!actor) return;
    const parsed = UpdateVerificationAssignmentSchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send({ error: parsed.error.flatten() });
    const { id } = request.params as { id: string }; const assignment = await dbService.verificationAssignments.updateStatus(id, actor.id, parsed.data.status);
    await dbService.audit.log({ actor_id: actor.id, event_type: `verification.assignment_${parsed.data.status}`, target_table: 'issue_clusters', target_id: assignment.cluster_id, payload: { assignment_id: id } }); return assignment;
  });
  ...
  app.post('/api/v1/verification', async (request, reply) => {
    ...
    try {
      ...
      if (assignment) await dbService.verificationAssignments.updateStatus(assignment.id, actor.id, 'completed');
  ...
  app.post('/api/v1/manifesto/generate', async (request, reply) => {
    ...
    if (actor.role === 'party_lead' && !party) return reply.status(403).send({ error: 'A verified party membership is required' });
    const grant = party ? await requireScope(actor, reply, 'party', party.id, 'manifesto.generate') : { id: 'platform-admin' }; if (!grant) return;
    const { areaId } = request.body as { areaId: string };
    if (!areaId) return reply.status(400).send({ error: 'areaId is required' });
    ...
    try {
      ...
      if (verifiedClusters.length === 0) return reply.status(409).send({ error: 'No verified clusters are ready for drafting' });
  ...
  app.get('/api/v1/manifesto/:areaId', async (request, reply) => {
    ...
    if (!manifesto) return reply.status(404).send({ error: 'No published manifesto found for this area' });
  ...
  app.get('/api/v1/public/manifestos/:areaId', async (request, reply) => {
    const { areaId } = request.params as { areaId: string };
    const manifesto = await dbService.manifestos.findLatestPublished(areaId);
    if (!manifesto) return reply.status(404).send({ error: 'No published manifesto for this area' });
  ...
  app.post('/api/v1/party/profiles/:id/verify', async (request, reply) => {
    const actor = await requireActor(request, reply, ['platform_admin']); if (!actor) return;
    const { id } = request.params as { id: string }; const profile = await dbService.partyProfiles.verify(id);
    await dbService.audit.log({ actor_id: actor.id, event_type: 'party.verify', target_table: 'party_profiles', target_id: id }); return profile;
  });

  app.post('/api/v1/party/memberships', async (request, reply) => {
    const actor = await requireActor(request, reply, ['platform_admin']); if (!actor) return;
    const parsed = AssignPartyMemberSchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send({ error: parsed.error.flatten() });
    ...
    await dbService.audit.log({ actor_id: actor.id, event_type: 'party.member_assign', target_table: 'party_profiles', target_id: parsed.data.party_id, payload: { user_id: parsed.data.user_id } }); return reply.status(201).send(membership);
  });
  ...
  app.post('/api/v1/party/promises/adopt', async (request, reply) => {
    ...
    if (!party) return reply.status(403).send({ error: 'A verified party profile and approved membership are required' });
    const grant = await requireScope(actor, reply, 'party', party.id, 'promise.adopt'); if (!grant) return;
  ...
  app.patch('/api/v1/party/promises/:id', async (request, reply) => {
    const actor = await requireActor(request, reply, ['party_lead', 'platform_admin']); if (!actor) return;
    const parsed = UpdatePartyPromiseSchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send({ error: parsed.error.flatten() });
    const { id } = request.params as { id: string }; const existing = await dbService.partyPromises.findUnique(id); if (!existing) return reply.status(404).send({ error: 'Promise not found' });
    let grant: { id: string } = { id: 'platform-admin' };
    if (actor.role === 'party_lead') { const party = await dbService.partyMemberships.findPartyForUser(actor.id); if (!party || party.id !== existing.party_id) return reply.status(403).send({ error: 'Promise belongs to another party profile' }); const activeGrant = await requireScope(actor, reply, 'party', party.id, 'promise.update'); if (!activeGrant) return; grant = activeGrant; }
    ...
    await dbService.audit.log({ actor_id: actor.id, event_type: 'promise.update', target_table: 'party_promises', target_id: id, payload: { ...parsed.data, grant_id: grant.id } }); return updated;
  });

  app.post('/api/v1/party/promises/:id/milestones', async (request, reply) => {
    const actor = await requireActor(request, reply, ['party_lead', 'department_officer', 'platform_admin']); if (!actor) return;
    const { id } = request.params as { id: string }; if (!(await dbService.partyPromises.findUnique(id))) return reply.status(404).send({ error: 'Promise not found' });
    const parsed = CreatePromiseMilestoneSchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send({ error: parsed.error.flatten() });
    ...
    await dbService.audit.log({ actor_id: actor.id, event_type: 'promise.milestone_create', target_table: 'party_promises', target_id: id, payload: { milestone_id: item.id, sequence: item.sequence } }); return reply.status(201).send(item);
  });

  app.patch('/api/v1/promise-milestones/:id', async (request, reply) => {
    const actor = await requireActor(request, reply, ['department_officer', 'volunteer', 'platform_moderator', 'platform_admin']); if (!actor) return;
    const parsed = UpdatePromiseMilestoneSchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send({ error: parsed.error.flatten() });
    const milestoneId = (request.params as { id: string }).id; const existing = await dbService.promiseMilestones.findUnique(milestoneId);
    ...
    if (isCitizenVerification && actor.role === 'department_officer') return reply.status(403).send({ error: 'Citizen verification must remain independent of the delivery owner' });
    if (isCitizenVerification && parsed.data.status !== 'verified') return reply.status(400).send({ error: 'The citizen verification milestone requires verified status' });
    try { const item = await dbService.promiseMilestones.update(milestoneId, { ...parsed.data, completed_at: parsed.data.completed_at ? new Date(parsed.data.completed_at) : undefined }); await dbService.audit.log({ actor_id: actor.id, event_type: 'promise.milestone_update', target_table: 'party_promises', target_id: item.party_promise_id, payload: { milestone_id: item.id, status: item.status, evidence_url: item.evidence_url } }); return item; }
    catch { return reply.status(404).send({ error: 'Milestone not found' }); }
  });

  app.get('/api/v1/public/promises/:id/accountability', async (request, reply) => {
    const record = await promiseAccountability((request.params as { id: string }).id); return record || reply.status(404).send({ error: 'Promise not found' });
  });

  app.put('/api/v1/party/promises/:id/verdict', async (request, reply) => {
    const actor = await requireActor(request, reply, ['citizen', 'volunteer', 'platform_admin']); if (!actor) return;
    const { id } = request.params as { id: string }; if (!(await dbService.partyPromises.findUnique(id))) return reply.status(404).send({ error: 'Promise not found' });
    const parsed = CitizenPromiseVerdictSchema.safeParse(request.body); if (!parsed.success) return reply.status(400).send({ error: parsed.error.flatten() });
    const item = await dbService.promiseVerdicts.upsert(id, actor.id, parsed.data.verdict, parsed.data.evidence_url);
    await dbService.audit.log({ actor_id: actor.id, event_type: 'promise.citizen_verdict', target_table: 'party_promises', target_id: id, payload: { verdict: item.verdict } }); return item;
  });
  ...
  app.post('/api/v1/manifesto/:id/publish', async (request, reply) => {
    ...
    if (actor.role === 'party_lead') {
      ...
      if (!party) return reply.status(403).send({ error: 'A verified party membership is required' });
      const grant = await requireScope(actor, reply, 'party', party.id, 'manifesto.publish'); if (!grant) return;
    }
  ...
  app.get('/api/v1/disputes', async (request, reply) => {
    ...
    if (!actor) return;
    const { promiseId } = request.query as { promiseId?: string };
  ...
  app.post('/api/v1/moderation/actions', async (request, reply) => {
    ...
    if (parsed.data.target_table === 'issues') {
      ...
      if (visibility) await dbService.issues.updateVisibility(parsed.data.target_id, visibility);
      if (status) await dbService.issues.updateStatus(parsed.data.target_id, status);
    }
  ...
  app.get('/api/v1/aggregates/areas/:areaId', async (request) => {
    ...
    const categoryMix = issues.reduce<Record<string, number>>((acc, issue) => ({ ...acc, [issue.category]: (acc[issue.category] ?? 0) + 1 }), {});
    const statusMix = promises.reduce<Record<string, number>>((acc, promise) => ({ ...acc, [promise.status]: (acc[promise.status] ?? 0) + 1 }), {});
    return {
      area_id: areaId,
      report_count: issues.length,
  ...
  app.get('/api/v1/exports/manifestos/:id.pdf', async (request, reply) => {
    ...
    if (!manifesto || !manifesto.is_published) return reply.status(404).send({ error: 'Published manifesto not found' });
  ...
  app.get('/api/v1/audit/page', async (request, reply) => {
    const actor = await requireActor(request, reply, ['platform_moderator', 'platform_admin']); if (!actor) return;
    const query = request.query as { page?: string; page_size?: string; event_type?: string; target_table?: string; search?: string };
    const pagination = PaginationSchema.safeParse(query); if (!pagination.success) return reply.status(400).send({ error: pagination.error.flatten() });
    let events = await dbService.audit.findMany();
    if (query.event_type) events = events.filter(event => event.event_type === query.event_type);
    if (query.target_table) events = events.filter(event => event.target_table === query.target_table);
    if (query.search) { const term = query.search.toLowerCase(); events = events.filter(event => JSON.stringify(event).toLowerCase().includes(term)); }
    const { page, page_size } = pagination.data; return { items: events.slice((page - 1) * page_size, page * page_size), page, page_size, total: events.length, last_updated: new Date() };
  });
