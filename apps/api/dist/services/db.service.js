...
class InMemoryDb {
    ...
    seedInitialData() {
        ...
        this.authorities.push({ id: '20000000-0000-4000-8000-000000000001', name: 'Municipal Roads Department', jurisdiction_area_id: ward12.id, category: 'roads', service_code: 'ROADS-POTHOLE', service_name: 'Road and pothole repair', description: 'Road surface, footpath, and pothole service requests.', active: true, created_at: authorityNow, updated_at: authorityNow }, { id: '20000000-0000-4000-8000-000000000002', name: 'Municipal Hydraulic Department', jurisdiction_area_id: ward12.id, category: 'water', service_code: 'WATER-SUPPLY', service_name: 'Water supply and waterlogging', description: 'Leaks, interrupted supply, and public waterlogging.', active: true, created_at: authorityNow, updated_at: authorityNow }, { id: '20000000-0000-4000-8000-000000000003', name: 'Municipal Solid Waste Department', jurisdiction_area_id: ward12.id, category: 'garbage', service_code: 'WASTE-COLLECTION', service_name: 'Waste collection', description: 'Missed collection, dumping, and sanitation requests.', active: true, created_at: authorityNow, updated_at: authorityNow });
        // Seed party profiles
        const party1 = {
        ...
        const manifesto = { id: '30000000-0000-4000-8000-000000000001', area_id: ward12.id, version: 1, is_published: true, published_at: new Date('2026-04-18'), published_by: defaultPartyLead.id, generation_provider: 'deterministic', generation_model: 'tgim-evidence-template-v1', source_cluster_ids: [defaultCluster.id], created_at: new Date('2026-04-12'), updated_at: new Date('2026-04-18') };
        const sourcePromise = { id: '30000000-0000-4000-8000-000000000002', manifesto_id: manifesto.id, cluster_id: defaultCluster.id, time_horizon: '1-year', title: 'A safer school route before the next monsoon', description: 'Repair the waterlogged school approach, clear stormwater inlets, and publish evidence before the next monsoon.', target_metric: 'Restore a safe, passable 90 metre school approach with verified drainage.', created_at: new Date('2026-04-18'), updated_at: new Date('2026-04-18') };
        const adopted = { id: '30000000-0000-4000-8000-000000000003', party_id: party1.id, source_promise_id: sourcePromise.id, adopted_title: sourcePromise.title, adopted_description: sourcePromise.description, target_metric: sourcePromise.target_metric, timeline: new Date('2026-08-31'), status: 'on_track', owner_department: 'Municipal Hydraulic Department', estimated_cost: '₹18–24 lakh departmental estimate', feasibility_notes: 'Drain clearance and surface regrading can be delivered before peak monsoon subject to work-order approval.', created_at: new Date('2026-04-18'), updated_at: new Date('2026-07-12') };
        this.manifestos.push(manifesto);
        this.promises.push(sourcePromise);
        this.partyPromises.push(adopted);
        this.scopeGrants.push({ id: crypto.randomUUID(), actor_id: defaultPartyLead.id, scope_type: 'party', scope_id: party1.id, capabilities: ['manifesto.generate', 'promise.adopt', 'promise.update', 'manifesto.publish'], issued_by: defaultAdmin.id, starts_at: new Date('2026-01-01'), created_at: new Date('2026-01-01') }, { id: crypto.randomUUID(), actor_id: defaultOfficer.id, scope_type: 'authority', scope_id: '20000000-0000-4000-8000-000000000002', capabilities: ['delivery.update', 'external_case.link', 'external_case.submit'], issued_by: defaultAdmin.id, starts_at: new Date('2026-01-01'), created_at: new Date('2026-01-01') });
...
export const dbService = {
    ...
    users: {
        ...
        findByClerkUserId: async (clerkUserId) => {
            if (isPrismaEnabled && prisma) {
                return prisma.user.findUnique({ where: { clerk_user_id: clerkUserId } });
            }
            return memoryDb.users.find(u => u.clerk_user_id === clerkUserId) || null;
        },
        findByIdentity: async (issuer, subject) => {
            if (isPrismaEnabled && prisma) {
    ...
    organizations: {
        create: async (data) => {
            if (isPrismaEnabled && prisma)
                return prisma.organization.create({ data: { ...data, active: true } });
            const now = new Date();
            const item = { id: crypto.randomUUID(), ...data, active: true, created_at: now, updated_at: now };
        ...
        findForUser: async (userId) => {
            if (isPrismaEnabled && prisma) {
                const memberships = await prisma.organizationMembership.findMany({ where: { user_id: userId }, include: { organization: true }, orderBy: { created_at: 'asc' } });
                return memberships.map((item) => ({ ...item.organization, membership_role: item.role }));
            }
        ...
        findMembership: async (organizationId, userId) => {
            if (isPrismaEnabled && prisma)
                return prisma.organizationMembership.findUnique({ where: { organization_id_user_id: { organization_id: organizationId, user_id: userId } } });
        ...
        addMembership: async (data) => {
            if (isPrismaEnabled && prisma)
                return prisma.organizationMembership.upsert({ where: { organization_id_user_id: { organization_id: data.organization_id, user_id: data.user_id } }, update: { role: data.role }, create: data });
            ...
            const now = new Date();
            const item = { id: crypto.randomUUID(), ...data, created_at: now, updated_at: now };
            memoryDb.organizationMemberships.push(item);
            return item;
        },
        createInvitation: async (data) => {
            if (isPrismaEnabled && prisma)
                return prisma.organizationInvitation.create({ data });
            const item = { id: crypto.randomUUID(), ...data, created_at: new Date() };
            memoryDb.organizationInvitations.push(item);
            return item;
        },
        findInvitationByTokenHash: async (tokenHash) => {
            if (isPrismaEnabled && prisma)
                return prisma.organizationInvitation.findUnique({ where: { token_hash: tokenHash } });
            return memoryDb.organizationInvitations.find(item => item.token_hash === tokenHash) ?? null;
        },
        acceptInvitation: async (id) => {
            if (isPrismaEnabled && prisma)
                return prisma.organizationInvitation.update({ where: { id }, data: { accepted_at: new Date() } });
            const item = memoryDb.organizationInvitations.find(value => value.id === id);
            if (!item)
                throw new Error('Invitation not found');
            item.accepted_at = new Date();
            return item;
        },
    },
    scopeGrants: {
        create: async (data) => {
            if (isPrismaEnabled && prisma)
                return prisma.actorScopeGrant.create({ data });
            const item = { id: crypto.randomUUID(), ...data, created_at: new Date() };
            memoryDb.scopeGrants.push(item);
            return item;
        },
        findActive: async (actorId, scopeType, scopeId, capability) => {
            const now = new Date();
            if (isPrismaEnabled && prisma)
                return prisma.actorScopeGrant.findFirst({
        ...
        revoke: async (id) => {
            if (isPrismaEnabled && prisma)
                return prisma.actorScopeGrant.update({ where: { id }, data: { revoked_at: new Date() } });
            const item = memoryDb.scopeGrants.find(grant => grant.id === id);
            if (!item)
                throw new Error('Scope grant not found');
            item.revoked_at = new Date();
            return item;
        },
        findForActor: async (actorId) => {
            if (isPrismaEnabled && prisma)
                return prisma.actorScopeGrant.findMany({ where: { actor_id: actorId }, orderBy: { created_at: 'desc' } });
    ...
    reportSharingConsents: {
        grant: async (data) => {
            if (isPrismaEnabled && prisma)
                return prisma.reportSharingConsent.upsert({
            ...
            const item = { id: crypto.randomUUID(), ...data, created_at: new Date() };
            memoryDb.reportSharingConsents.push(item);
            return item;
        },
        findActive: async (issueId, authorityId, purpose = 'external_case_submission') => {
            const now = new Date();
            if (isPrismaEnabled && prisma)
                return prisma.reportSharingConsent.findFirst({ where: { issue_id: issueId, authority_id: authorityId, purpose, revoked_at: null, OR: [{ expires_at: null }, { expires_at: { gt: now } }] } });
    ...
    authorities: {
        create: async (data) => {
            if (isPrismaEnabled && prisma)
                return prisma.civicAuthority.create({ data });
            const now = new Date();
            const authority = { id: crypto.randomUUID(), ...data, created_at: now, updated_at: now };
            memoryDb.authorities.push(authority);
            return authority;
        },
        findMany: async (filters = {}) => {
            if (isPrismaEnabled && prisma)
                return prisma.civicAuthority.findMany({ where: { jurisdiction_area_id: filters.area_id, category: filters.category, active: filters.active }, orderBy: { name: 'asc' } });
        ...
        findUnique: async (id) => {
            if (isPrismaEnabled && prisma)
                return prisma.civicAuthority.findUnique({ where: { id } });
            return memoryDb.authorities.find(item => item.id === id) ?? null;
        },
        route: async (areaId, category, latitude, longitude) => {
            if (isPrismaEnabled && prisma && latitude !== undefined && longitude !== undefined)
    ...
    pincodes: {
        findMany: async () => {
            if (isPrismaEnabled && prisma) {
                return prisma.pincodeBoundary.findMany({ orderBy: { pincode_code: 'asc' } });
            }
            return memoryDb.pincodeBoundaries;
        },
        findByCode: async (code) => {
            if (isPrismaEnabled && prisma) {
                return prisma.pincodeBoundary.findUnique({ where: { pincode_code: code } });
            }
            return memoryDb.pincodeBoundaries.find(p => p.pincode_code === code) ?? null;
        },
        resolveByCoordinates: async (latitude, longitude) => {
            if (isPrismaEnabled && prisma) {
    ...
    outboxEvents: {
        ...
        findMany: async () => {
            if (isPrismaEnabled && prisma) {
                const items = await prisma.outboxEvent.findMany({ orderBy: { created_at: 'asc' } });
                return items.map((item) => ({ ...item, event_id: item.id }));
            }
            return memoryDb.outboxEvents.map(item => ({ ...item, event_id: item.id }));
        },
        findById: async (id) => {
            if (isPrismaEnabled && prisma) {
                const item = await prisma.outboxEvent.findUnique({ where: { id } });
                return item ? { ...item, event_id: item.id } : null;
            }
            const item = memoryDb.outboxEvents.find(event => event.id === id);
            return item ? { ...item, event_id: item.id } : null;
        },
        markDispatched: async (id, providerMessageId) => {
            if (isPrismaEnabled && prisma) {
            ...
            if (item) {
                ...
                item.provider_message_id = providerMessageId ?? null;
        ...
        markFailed: async (id, error) => {
            ...
            if (item) {
                item.status = 'failed';
                item.last_safe_error = error.slice(0, 1000);
                item.attempt_count += 1;
            }
        },
        markAcknowledged: async (id) => {
            if (isPrismaEnabled && prisma) {
                await prisma.outboxEvent.update({ where: { id }, data: { status: 'acknowledged', acknowledged_at: new Date(), last_safe_error: null } });
                return;
            }
            const item = memoryDb.outboxEvents.find(event => event.id === id);
            if (item) {
    ...
    externalCases: {
        create: async (data) => {
            if (isPrismaEnabled && prisma) {
                const item = await prisma.externalGrievanceCase.create({ data, include: { authority: true } });
                return { ...item, authority_name: item.authority.name };
            }
            const authority = memoryDb.authorities.find(item => item.id === data.authority_id);
            if (!authority)
                throw new Error('Authority not found');
            const now = new Date();
            const item = { id: crypto.randomUUID(), ...data, authority_name: authority.name, created_at: now, updated_at: now };
            memoryDb.externalCases.push(item);
            return item;
        },
        findByIssue: async (issueId) => {
            if (isPrismaEnabled && prisma)
                return (await prisma.externalGrievanceCase.findMany({ where: { issue_id: issueId }, include: { authority: true }, orderBy: { submitted_at: 'desc' } })).map(item => ({ ...item, authority_name: item.authority.name }));
            return memoryDb.externalCases.filter(item => item.issue_id === issueId);
        },
        findUnique: async (id) => {
            if (isPrismaEnabled && prisma) {
                const item = await prisma.externalGrievanceCase.findUnique({ where: { id }, include: { authority: true } });
                return item ? { ...item, authority_name: item.authority.name } : null;
            }
            return memoryDb.externalCases.find(item => item.id === id) ?? null;
        },
        update: async (id, data) => {
            if (isPrismaEnabled && prisma) {
                const item = await prisma.externalGrievanceCase.update({ where: { id }, data, include: { authority: true } });
                return { ...item, authority_name: item.authority.name };
            }
            const item = memoryDb.externalCases.find(value => value.id === id);
            if (!item)
                throw new Error('External case not found');
    ...
    externalCaseDocuments: {
        create: async (data) => {
            if (isPrismaEnabled && prisma)
                return prisma.externalCaseDocument.create({ data });
            const item = { id: crypto.randomUUID(), ...data, created_at: new Date() };
            memoryDb.externalCaseDocuments.push(item);
            return item;
        },
        findByCase: async (caseId, publicOnly = false) => {
            if (isPrismaEnabled && prisma)
                return prisma.externalCaseDocument.findMany({ where: { external_case_id: caseId, ...(publicOnly ? { is_public: true } : {}) }, orderBy: { created_at: 'desc' } });
    ...
    externalCaseAppeals: {
        create: async (data) => {
            if (isPrismaEnabled && prisma)
                return prisma.externalCaseAppeal.create({ data: { ...data, status: 'submitted' } });
            const now = new Date();
            const item = { id: crypto.randomUUID(), ...data, status: 'submitted', created_at: now, updated_at: now };
            memoryDb.externalCaseAppeals.push(item);
            return item;
        },
        findByCase: async (caseId) => {
            if (isPrismaEnabled && prisma)
                return prisma.externalCaseAppeal.findMany({ where: { external_case_id: caseId }, orderBy: { created_at: 'desc' } });
    ...
    issues: {
        ...
        findUnique: async (id) => {
            if (isPrismaEnabled && prisma) {
                const issues = await prisma.$queryRaw(Prisma.sql `
        ...
        findMany: async (filters) => {
            if (isPrismaEnabled && prisma) {
                ...
                if (filters.area_id)
                    conditions.push(Prisma.sql `area_id = ${filters.area_id}::uuid`);
                if (filters.category)
        ...
        updateVisibility: async (issueId, visibility) => {
            if (isPrismaEnabled && prisma) {
                const result = await prisma.$queryRaw(Prisma.sql `
    ...
    verificationAssignments: {
        assign: async (data) => {
            if (isPrismaEnabled && prisma)
                return prisma.verificationAssignment.upsert({ where: { cluster_id_volunteer_id: { cluster_id: data.cluster_id, volunteer_id: data.volunteer_id } }, update: { status: 'assigned', safety_notes: data.safety_notes, due_at: data.due_at }, create: data });
            ...
            if (existing) {
                Object.assign(existing, data, { status: 'assigned', updated_at: new Date() });
                return existing;
            }
            const now = new Date();
            const item = { id: crypto.randomUUID(), ...data, status: 'assigned', created_at: now, updated_at: now };
            memoryDb.verificationAssignments.push(item);
            return item;
        },
        findByVolunteer: async (volunteerId) => {
            if (isPrismaEnabled && prisma)
                return prisma.verificationAssignment.findMany({ where: { volunteer_id: volunteerId, status: { not: 'cancelled' } }, orderBy: { due_at: 'asc' } });
            return memoryDb.verificationAssignments.filter(item => item.volunteer_id === volunteerId && item.status !== 'cancelled');
        },
        updateStatus: async (id, volunteerId, status) => {
            if (isPrismaEnabled && prisma)
                return prisma.verificationAssignment.update({ where: { id, volunteer_id: volunteerId }, data: { status } });
            const item = memoryDb.verificationAssignments.find(assignment => assignment.id === id && assignment.volunteer_id === volunteerId);
            if (!item)
    ...
    manifestos: {
        ...
        findLatestPublished: async (areaId) => {
            if (isPrismaEnabled && prisma) {
                return prisma.manifesto.findFirst({
        ...
        findUnique: async (id) => {
            if (isPrismaEnabled && prisma)
                return prisma.manifesto.findUnique({ where: { id } });
            return memoryDb.manifestos.find(item => item.id === id) ?? null;
        },
        publish: async (id, publisherId) => {
            if (isPrismaEnabled && prisma) {
        ...
        setGenerationMetadata: async (id, data) => {
            if (isPrismaEnabled && prisma)
                return prisma.manifesto.update({ where: { id }, data: data });
    ...
    partyProfiles: {
        findMany: async () => {
            if (isPrismaEnabled && prisma)
                return prisma.partyProfile.findMany({ orderBy: { name: 'asc' } });
            return memoryDb.partyProfiles;
        },
        verify: async (id) => {
            if (isPrismaEnabled && prisma)
                return prisma.partyProfile.update({ where: { id }, data: { is_verified: true } });
            const party = memoryDb.partyProfiles.find(item => item.id === id);
            if (!party)
                throw new Error('Party profile not found');
            party.is_verified = true;
            return party;
        },
    },
    partyMemberships: {
        findPartyForUser: async (userId) => {
            if (isPrismaEnabled && prisma) {
                const membership = await prisma.partyMembership.findFirst({ where: { user_id: userId, is_approved: true }, include: { party: true } });
        ...
        assign: async (data) => {
            if (isPrismaEnabled && prisma)
                return prisma.partyMembership.upsert({ where: { user_id_party_id: { user_id: data.user_id, party_id: data.party_id } }, update: { title: data.title, is_approved: true }, create: { ...data, is_approved: true } });
            ...
            const membership = { id: crypto.randomUUID(), ...data, is_approved: true, created_at: new Date() };
            memoryDb.partyMemberships.push(membership);
            return membership;
        },
    },
    ...
    partyPromises: {
        ...
        update: async (id, data) => {
            if (isPrismaEnabled && prisma)
                return prisma.partyPromise.update({ where: { id }, data: data });
            const promise = memoryDb.partyPromises.find(item => item.id === id);
            if (!promise)
                throw new Error('Party promise not found');
            Object.assign(promise, data, { updated_at: new Date() });
            return promise;
        }
    },
    promiseMilestones: {
        create: async (data) => {
            if (isPrismaEnabled && prisma)
                return prisma.promiseMilestone.create({ data: { ...data, status: 'pending' } });
            const now = new Date();
            const item = { id: crypto.randomUUID(), ...data, status: 'pending', created_at: now, updated_at: now };
            memoryDb.promiseMilestones.push(item);
            return item;
        },
        findByPromise: async (promiseId) => {
            if (isPrismaEnabled && prisma)
                return prisma.promiseMilestone.findMany({ where: { party_promise_id: promiseId }, orderBy: { sequence: 'asc' } });
            return memoryDb.promiseMilestones.filter(item => item.party_promise_id === promiseId).sort((a, b) => a.sequence - b.sequence);
        },
        findUnique: async (id) => {
            if (isPrismaEnabled && prisma)
                return prisma.promiseMilestone.findUnique({ where: { id } });
            return memoryDb.promiseMilestones.find(item => item.id === id) ?? null;
        },
        update: async (id, data) => {
            if (isPrismaEnabled && prisma)
                return prisma.promiseMilestone.update({ where: { id }, data });
            const item = memoryDb.promiseMilestones.find(value => value.id === id);
            if (!item)
    ...
    promiseVerdicts: {
        upsert: async (partyPromiseId, userId, verdict, evidenceUrl) => {
            if (isPrismaEnabled && prisma)
                return prisma.citizenPromiseVerdict.upsert({ where: { party_promise_id_user_id: { party_promise_id: partyPromiseId, user_id: userId } }, update: { verdict, evidence_url: evidenceUrl }, create: { party_promise_id: partyPromiseId, user_id: userId, verdict, evidence_url: evidenceUrl } });
            const existing = memoryDb.promiseVerdicts.find(item => item.party_promise_id === partyPromiseId && item.user_id === userId);
            if (existing) {
                existing.verdict = verdict;
                existing.evidence_url = evidenceUrl;
                existing.updated_at = new Date();
                return existing;
            }
            const now = new Date();
            const item = { id: crypto.randomUUID(), party_promise_id: partyPromiseId, user_id: userId, verdict, evidence_url: evidenceUrl, created_at: now, updated_at: now };
            memoryDb.promiseVerdicts.push(item);
            return item;
        },
        findByPromise: async (promiseId) => {
            if (isPrismaEnabled && prisma)
                return prisma.citizenPromiseVerdict.findMany({ where: { party_promise_id: promiseId } });
    ...
    volunteerApplications: {
        ...
        findMany: async () => {
            if (isPrismaEnabled && prisma)
                return prisma.volunteerApplication.findMany({ orderBy: { created_at: 'desc' } });
            return memoryDb.volunteerApplications;
        },
        review: async (id, reviewerId, status, reviewNotes) => {
            if (isPrismaEnabled && prisma) {
                return prisma.$transaction(async (tx) => {
                    ...
                    if (status === 'approved')
                        await tx.user.update({ where: { id: application.user_id }, data: { role: 'volunteer' } });
                    return application;
                });
            }
            ...
            if (status === 'approved')
                await dbService.users.updateRole(item.user_id, 'volunteer');
            return item;
        },
    },
    disputes: {
        create: async (data) => {
            if (isPrismaEnabled && prisma)
                return prisma.deliveryDispute.create({ data: { ...data, status: 'open' } });
            const now = new Date();
            const item = { id: crypto.randomUUID(), ...data, status: 'open', created_at: now, updated_at: now };
            memoryDb.disputes.unshift(item);
            return item;
        },
        findMany: async (promiseId) => {
            if (isPrismaEnabled && prisma)
                return prisma.deliveryDispute.findMany({ where: promiseId ? { party_promise_id: promiseId } : {}, orderBy: { created_at: 'desc' } });
            return promiseId ? memoryDb.disputes.filter(item => item.party_promise_id === promiseId) : memoryDb.disputes;
        },
        findPublicByPromise: async (promiseId) => {
            if (isPrismaEnabled && prisma)
                return prisma.deliveryDispute.findMany({ where: { party_promise_id: promiseId, is_public: true, published_at: { not: null } }, orderBy: { published_at: 'desc' } });
            return memoryDb.disputes.filter(item => item.party_promise_id === promiseId && item.is_public && item.published_at);
        },
        resolve: async (id, resolverId, status, notes, publication) => {
            ...
            if (isPrismaEnabled && prisma)
                return prisma.deliveryDispute.update({ where: { id }, data: update });
    ...
    notificationPreferences: {
        get: async (userId) => {
            if (isPrismaEnabled && prisma)
                return prisma.notificationPreference.findUnique({ where: { user_id: userId } });
            return memoryDb.notificationPreferences.find(item => item.user_id === userId) ?? null;
        },
        upsert: async (data) => {
            if (isPrismaEnabled && prisma)
                return prisma.notificationPreference.upsert({ where: { user_id: data.user_id }, create: data, update: data });
            const index = memoryDb.notificationPreferences.findIndex(item => item.user_id === data.user_id);
            if (index >= 0)
                memoryDb.notificationPreferences[index] = data;
            else
                memoryDb.notificationPreferences.push(data);
            return data;
        },
        findPromiseSubscribers: async () => {
            if (isPrismaEnabled && prisma)
                return prisma.notificationPreference.findMany({ where: { promise_updates: true } });
    ...
    notifications: {
        create: async (data) => {
            ...
            const item = { id: crypto.randomUUID(), ...data, created_at: new Date() };
            memoryDb.notifications.unshift(item);
            return item;
        },
        findByUser: async (userId) => {
            if (isPrismaEnabled && prisma)
                return prisma.userNotification.findMany({ where: { user_id: userId }, orderBy: { created_at: 'desc' }, take: 100 });
            return memoryDb.notifications.filter(item => item.user_id === userId);
        },
        markRead: async (id, userId) => {
            if (isPrismaEnabled && prisma)
                return prisma.userNotification.update({ where: { id, user_id: userId }, data: { read_at: new Date() } });
            const item = memoryDb.notifications.find(notification => notification.id === id && notification.user_id === userId);
            if (!item)
    ...
    moderation: {
        ...
        findMany: async () => {
            if (isPrismaEnabled && prisma)
                return prisma.moderationAction.findMany({ orderBy: { created_at: 'desc' } });
            return memoryDb.moderationActions;
        },
    },
    jobs: {
        enqueue: async (type, payload, runAfter = new Date()) => {
            if (isPrismaEnabled && prisma)
                return prisma.backgroundJob.create({ data: { type, payload, run_after: runAfter } });
            const now = new Date();
            const job = { id: crypto.randomUUID(), type, payload, status: 'pending', attempts: 0, max_attempts: 5, run_after: runAfter, created_at: now, updated_at: now };
            memoryDb.jobs.push(job);
            return job;
        },
        claim: async () => {
            if (isPrismaEnabled && prisma) {
                const rows = await prisma.$queryRaw(Prisma.sql `UPDATE background_jobs SET status = 'running', locked_at = now(), attempts = attempts + 1, updated_at = now() WHERE id = (SELECT id FROM background_jobs WHERE status IN ('pending','failed') AND run_after <= now() AND attempts < max_attempts ORDER BY run_after ASC FOR UPDATE SKIP LOCKED LIMIT 1) RETURNING *`);
                return rows[0] ?? null;
            }
        ...
        complete: async (id) => {
            if (isPrismaEnabled && prisma) {
                await prisma.backgroundJob.update({ where: { id }, data: { status: 'completed', locked_at: null } });
                return;
            }
            const job = memoryDb.jobs.find(item => item.id === id);
            if (job)
                Object.assign(job, { status: 'completed', locked_at: undefined, updated_at: new Date() });
        },
        fail: async (id, error) => {
            if (isPrismaEnabled && prisma) {
                const job = await prisma.backgroundJob.findUnique({ where: { id } });
                if (job)
                    await prisma.backgroundJob.update({ where: { id }, data: { status: job.attempts >= job.max_attempts ? 'dead_letter' : 'failed', last_error: error, locked_at: null, run_after: new Date(Date.now() + Math.min(60_000, 1000 * 2 ** job.attempts)) } });
                return;
            }
            const job = memoryDb.jobs.find(item => item.id === id);
            if (job)
                Object.assign(job, { status: job.attempts >= job.max_attempts ? 'dead_letter' : 'failed', last_error: error, locked_at: undefined, run_after: new Date(Date.now() + Math.min(60_000, 1000 * 2 ** job.attempts)), updated_at: new Date() });
        },
        findMany: async () => {
            if (isPrismaEnabled && prisma)
                return prisma.backgroundJob.findMany({ orderBy: { created_at: 'desc' }, take: 100 });
